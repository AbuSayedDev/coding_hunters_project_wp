...
add_action( 'elementor/init', 'codinghunters_register_google_maps_skin' );

function codinghunters_register_google_maps_skin() {
	
	class Codinghunters_Google_Maps_Skin extends Elementor\Skin_Base {
		...
		public function get_id() {
			return 'codinghunters_google_maps';
		}

		public function get_title() {
			return __( 'codinghunters map', 'codinghunters' );
		}

		public function register_controls( $controls ) {
				$controls->remove_control( 'address' );

				$default_address = __( 'London Eye, London, United Kingdom', 'codinghunters' );

				$controls->add_control(
					'address',
					[
						'label' => __( 'Address', 'codinghunters' ),
		...
		public function register_skin_controls( $controls ) {
			...
			$controls->add_control(
				'important_note',
				[
					'label' => '<strong>' . __( 'Note', 'codinghunters' ) . '</strong>',
					'condition' => [
						'_skin' => $this->get_id(),
					],							
					'type' => \Elementor\Controls_Manager::RAW_HTML,
					'raw' => sprintf( __( 'Please go here %s to find the latitude and longitute for your address.', 'codinghunters' ), '<a target="_blank" href="https://www.latlong.net/convert-address-to-lat-long.html">' . __( 'here', 'codinghunters' ) . '</a>'),
			...
			$controls->add_control(
				'api_key',
				[
					'label' => __( 'API key', 'codinghunters' ),
					'type' => \Elementor\Controls_Manager::TEXT,
					'condition' => [
						'_skin' => $this->get_id(),
					],
					'description' => sprintf( __( 'Please go here %s to get your Google Maps API Key.', 'codinghunters' ), '<a target="_blank" href="https://developers.google.com/maps/documentation/javascript/get-api-key">' . __( 'here', 'codinghunters' ) . '</a>'),
					'placeholder' => '',
					'default' => '',
					'label_block' => true,
				]
			...
			$controls->add_control(
				'lat',
				[
					'label' => __( 'Latitude', 'codinghunters' ),
					'type' => \Elementor\Controls_Manager::TEXT,
					'condition' => [
						'_skin' => $this->get_id(),
					],							
			...
			$controls->add_control(
				'long',
				[
					'label' => __( 'Longitude', 'codinghunters' ),
					'type' => \Elementor\Controls_Manager::TEXT,
					'condition' => [
						'_skin' => $this->get_id(),
					],							
        ...
        <script type="text/javascript">
            ...
            function init() {
                ...
                var mapElement = document.getElementById('codinghunters-map');
...
add_action( 'elementor/widget/google_maps/skins_init', function( $widget ) {
   $widget->add_skin( new Codinghunters_Google_Maps_Skin( $widget ) );
} );
