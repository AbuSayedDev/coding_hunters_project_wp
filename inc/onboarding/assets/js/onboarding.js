...
/* global codinghunters_plugin_helper */
/* global console */
	...
	function ( $ ) {
		$.pluginInstall = {
			...
			'handleInstall': function () {
				var self = this;
				$( 'body' ).on( 'click', '.codinghunters-install-plugin', function ( e ) {
					e.preventDefault();
					var button   = $( this );
					...
					wp.updates.installPlugin(
						{
							slug: slug,
							success: function () {
								button.text( codinghunters_plugin_helper.activating + '...' );
...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.btn-get-started' ).click( function ( e ) {
		...
		// Change button text.
		jQuery( this ).text( codinghunters_redirect_demo_page.btn_text );
		...
		var data = {
			...
			security : codinghunters_redirect_demo_page.nonce,
			state      : state,
		};
		...
		$.ajax({
			...
			success:function( response ) {
				extra_uri = '';
				if ( jQuery( '.codinghunters-message-close' ).length ) {
					dismiss_nonce = jQuery( '.codinghunters-message-close' ).attr( 'href' ).split( '_codinghunters_notice_nonce=' )[1];
					extra_uri     = '&_codinghunters_notice_nonce=' + dismiss_nonce;
				}
